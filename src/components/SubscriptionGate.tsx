...
export default function SubscriptionGate({ children, onVerified }: SubscriptionGateProps) {
    ...
    const handleSignIn = async () => {
        try {
            ...
            console.log('🔵 Callback URL will be:', currentUrl);
        ...
        <div className="bg-white rounded-lg shadow-lg p-8">
            <div className="max-w-2xl mx-auto text-center">
                ...
                <div className="mb-8">
                    {verificationState === 'checking' || isVerifying ? (
                        ...
                        <div className="space-y-3">
                            <p className="text-gray-600">
                                You must have a gmail account and subscribed to Ndigbo Viva channel to view this content.
                            </p>
                        </div>
                    ) : verificationState === 'not-subscribed' ? (
                        <div className="space-y-3">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                    {verificationState === 'not-signed-in' && (
                        <>
                            ...
                            <button
                                ...
                                className="border-2 border-brand-gold text-brand-gold px-8 py-3 rounded-lg font-semibold hover:bg-brand-gold hover:text-white transition"
                            >
                    ...
                    {verificationState === 'error' && (
                        <>
                            {/* Show Sign In button if error is related to authentication */}
                            ...
                            {/* Show Sign In Again button for YouTube API permission errors */}
                            {errorMessage.includes('YouTube API access not granted') && (
                                    <button
                                        onClick={async () => {
                            ...
                            {/* Show Try Again button for other errors or as a fallback */}
                            {(!errorMessage.includes('not authenticated') && 
                              !errorMessage.includes('no access token') && 
                              !errorMessage.includes('Not authenticated') &&
                              !errorMessage.includes('YouTube API access not granted')) && (
